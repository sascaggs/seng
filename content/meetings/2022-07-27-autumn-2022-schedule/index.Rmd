...
After a welcome meeting on 9/9, we will meet to discuss *research methods and design*, focusing on methods used by social scientists to collect complete and personal networks. We will follow this meeting with a refresher on how to work with *networks in* `R`. The next two meetings will involve a deep dive into *Exponential Random Graph Models* (ERGMs). In these meetings we will work closely with the `statnet` suite of packages to fit, summarize, diagnose, and interpret ERGMs. In he final two meetings will explore some advanced topics including *Bayesian inference* for networks and *community detection* algorithms for identifying groups. 
...
```{r, echo=F, message=F, warning=F,fig.height=10, fig.width=5, fig.align='left'}
library(tidyverse)
